...
def preprocessing_data(data_path):
    #df = pd.read_csv(data_path, sep="\t", header=None, names=["text", "POS", "Chunk", "label"], quoting=3)
    dataset = {
        "doc": [],
        "labels": []
    }
...
def train(model, num_epochs, loss_function, optimizer,
    ...
    for epoch in range(num_epochs):
        ...
        for embedding, length, label in tqdm(train_dataloader):
            ...
            predictions = predictions.view(-1, predictions.shape[-1])  # (batch_size * seq_length, num_classes)
            label = label.view(-1)  # (batch_size * seq_length)
...
dev_dataset = preprocessing_data(dev_path)
train_dataset = preprocessing_data(train_path)
test_dataset = preprocessing_data(test_path)
...
def run(lr):
    ...
    visualize_data(f1_scores, "F1 scores with",lr)
